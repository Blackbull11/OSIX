...
$sources = $bdd->query("SELECT * FROM sources")->fetchAll();
...
$users = $bdd->query("SELECT * FROM users")->fetchAll();
$sources = $bdd->query("SELECT * FROM sources")->fetchAll();
$toolcategories = $bdd->query("SELECT * FROM sourcelists")->fetchAll();
...
if (isset($_POST['tool_id'])){
    ...
    $req_date = "SELECT * FROM sources WHERE id = :id AND DATE_ADD(date, INTERVAL 3 HOUR) >= NOW()";
...
//        - Sinon il est juste caché
    ...
    if ($stmt_date->rowCount() > 0) { //On ne rentre jamais dans cette condition ?
        ...
        $cat_links = $cat_stmt->fetchAll()[0]['sources'];
        ...
        $list_query = 'SELECT sources FROM sourcelists WHERE id = :id';
        $list_stmt = $bdd->prepare($list_query);
        $list_stmt->execute(['id' => $tool_cat]);
        $cat_links = $list_stmt->fetchAll()[0]['sources'];
