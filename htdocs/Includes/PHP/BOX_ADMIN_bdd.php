...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['key']) && $_POST['key'] == 'update_tool_recommended' ) {
    // Récupérer l'ID de l'outil et le statut "recommended"
    if (isset($_POST['toolId']) && isset($_POST['recommended'])) {
        $toolId = $_POST['toolId'];
        $isRecommended = $_POST['recommended'];

        // Mettre à jour l'état "recommended" dans la base de données
        $query = "UPDATE tools SET user0 = :recommended WHERE id = :toolId";
        $stmt = $bdd->prepare($query);
...
//        $filteredToolIds = $definite_filtered_Ids;
        ...
        if ($results) {
            foreach ($results as $tool) {
                echo '<tr>';
                $checked = $tool['user0']? 'checked' : '';
                echo '<td style="text-align: center"><input type="checkbox" id="tool-checkbox" '.$checked.' onclick="Update_Tool_Recommended(this, '.$tool['id'].')"></td>';
                if ($tool['image'] == null) {
                ...
                echo '<td class="url-column"><a target="_blank" href="' . $tool['url'] . '" target="_blank">' . $tool['url'] . '</a></td>';
